...
networks:
  internal:
    # internal: true を設定するとコンテナからの外部インターネットアクセスも遮断される。
    # 上流サービスが外部APIを呼び出す必要がある場合は設定しないこと。
    # 上流サービスのネットワーク隔離は ports: を書かないことで実現しているため、
    # internal: true は必須ではない。
```
...
**データの永続化について**:
- `./data:/var/lib/auth-proxy` のバインドマウントにより、DBファイルはホスト側の `./data/` ディレクトリに保存されます。
- `docker compose down` でコンテナを削除してもデータは `./data/` に残ります（`AUTH_PROXY_DB_PATH` はこのマウント先に合わせて設定すること）
- `.env.auth-proxy` ファイルはホスト上に置くだけでOKです。コンテナ再作成で消えることはありません。
