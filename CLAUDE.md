...
**`ApiTokenStoreDb::revoke_any` (and `list_all`) have exactly two trusted caller categories (Phase API-4, R9/R10).**  
Unlike `revoke` (which requires a matching `user_id`), `revoke_any` performs no ownership check — that's the point, since an admin must be able to revoke any user's token. It is safe only because every caller is either (a) an admin-gated HTTP handler under `src/handlers/admin/` (`handlers/admin/tokens.rs::post_admin_token_revoke`/`get_admin_tokens`), already checked by the same `role == "admin" && auth_method == "session"` guard as every other admin handler, or (b) the CLI under `src/cli/token.rs`, which has no `auth_method`/session concept at all and instead relies on OS-level trust (same trust boundary as `init-admin`/`hash`/`list`). Calling either function from any other HTTP handler would let a user revoke or enumerate another user's tokens. See ADR 0003.
